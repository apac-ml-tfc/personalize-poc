"""Diagnostic utilities for validating data prepared for Amazon Personalize"""
...
def describe_categorical(sr, name=''):
    print(f"\n=== {name} top {CATS_FREQ_HEAD} categories ===")
    ...
    if len(cats_freq) > LOGLOG_MIN_CATS and rmse < LOGLOG_RMSE_THRESHOLD:
        if slope > LOGLOG_HEAVY_TAIL:
            warnings.warn(
                f"""Heavy-tail {name} distributions are usually hard to learn (slope={slope})!
                Consider rolling up {name} or dropping its rare values.
                """
            )
        elif slope < LOGLOG_HEAD_HEAVY:
            warnings.warn(
                f"""Head-heavy {name} distributions are usually uninteresting or spammy (slope={slope})!
                Consider using finer-grade {name} or thresholding its dominate values.
                """
            )
    ...
    Y, X = [
        ss.coo_matrix(
            (
                ...
                (df_cnt[df_cnt["_bs"] == split]["_i"], df_cnt[df_cnt["_bs"] == split]["_j"])
            ),
            shape=(len(index), N)
        ).tocsr()
        for split in [0, 1]
...
def diagnose_interactions(df):
    print(f"\n=== Interactions table, original shape={df.shape} ===\n")
    df = df.copy()
    ...
    if dup_rate > DUP_RATE_THRESHOLD:
        warnings.warn(
            f"""High duplication rate ({dup_rate:.1%})!
    ...
    if repeat_rate > REPEAT_RATE_THRESHOLD:
        warnings.warn(
            f"""High rate of repeated consumptions ({repeat_rate:.1%})!
    ...
    for method in TEMPORAL_LOSS_METHODS:
        ...
        pl.title(f"{method} loss at different frequencies")
        pl.grid()
    ...
    for method in TEMPORAL_LOSS_METHODS:
        for freq in TEMPORAL_FREQUENCY:
            ...
            pl.plot(
                bootstrap_loss.iloc[-TEMPORAL_PLOT_LIMIT:],
                ".--",
                label=f"boostrap baseline={loss_fmt.format(avg_loss)}",
            )
            ...
            for hist_len in ROLLING_HISTORY_LEN:
                ...
                pl.plot(
                    temporal_loss.iloc[-TEMPORAL_PLOT_LIMIT:],
                    ".-",
                    label=f"hist={hist_len} * {freq}, avg={loss_fmt.format(avg_loss)}",
                )

            pl.gca().yaxis.set_major_formatter(pl.FuncFormatter(lambda y, _: loss_fmt.format(y)))

            pl.title(f"{freq} {method} from rolling history (lower is better)")
            pl.grid()
            pl.gcf().autofmt_xdate()
...
def diagnose_users(df, users):
    print(f"\n=== Users table, original shape={users.shape} ===\n")
    users = users.copy()
    ...
    if missing_rate > NA_RATE_THRESHOLD:
        warnings.warn(f"High missing rate of all user meta-data ({missing_rate:%})!")
    ...
    if coldstart_rate > COLDSTART_RATE_THRESHOLD:
        warnings.warn(f"High user coldstart rate ({coldstart_rate:%})!")

    describe_dataframe(users)


def diagnose_items(df, items):
    print(f"\n=== Items table, original shape={items.shape} ===\n")
    items = items.copy()
    ...
    if missing_rate > NA_RATE_THRESHOLD:
        warnings.warn(f"High missing rate of all item meta-data ({missing_rate:%})!")
    ...
    if coldstart_rate > NA_RATE_THRESHOLD:
        warnings.warn(f"High item coldstart rate ({coldstart_rate:%})!")
